...
function encodeXML(LabTech_Expansion) {
  return new Promise((resolve, reject) => {
    ...
    .then(LabTech_Expansion => {
      ...
      return traverse(LabTech_Expansion, encodeTraverseHandler)
...
function decodeXML(scriptXML) {
  return new Promise((resolve, reject) => {
    ...
    .then(parsed => traverse(parsed.LabTech_Expansion, decodeTraverseHandler))
    .then(LabTech_Expansion => {
      const valid = validate(LabTech_Expansion);
      if (valid) {
 ...
 */
function traverse(PackedScript, handler, promises) {
  if (!promises) {
    promises = [];
  }
  ...
  if (_.isArray(PackedScript)) {
    PackedScript.forEach(packed => traverse(packed, handler, promises));
  }
  ...
  if (_.isArray(PackedScript && PackedScript.PackedScript)) {
    PackedScript.PackedScript.forEach(packed => traverse(packed, handler, promises));
  }

  // if there's a single packed script
  if (PackedScript.PackedScript) {
    promises.push(traverse(PackedScript.PackedScript, handler));
  }
...
function interpolateScriptData(PackedScript) {
  ...
  ScriptSteps.forEach(step => {
    ...
    step = Object.assign(step, {
      ActionObject: Actions[step.Action],
      ContinueObject: Continues[step.Continue],
      FunctionObject: FunctionDef,
      OsLimitObject: OsLimits[step.OsLimit],
    });
  });
